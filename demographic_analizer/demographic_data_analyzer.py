...
def calculate_demographic_data(print_data=True):
    ...
    higher_education = round(df.loc[df['education'].isin(['Bachelors','Masters','Doctorate'])].shape[0]/df.shape[0]*100,1)
    lower_education = df.loc[~df['education'].isin(['Bachelors','Masters','Doctorate'])].shape[0]/df.shape[0]*100
    ...
    num_min_workers = df.loc[(df['hours-per-week']==df['hours-per-week'].min()) & (df['salary']=='>50K')].shape[0]/df.loc[(df['hours-per-week']==df['hours-per-week'].min())].shape[0]*100
    ...
    highest_earning_country_percentage = df.loc[(df['salary']=='>50K')]['native-country'].value_counts().max()/df.loc[(df['salary']=='>50K')].shape[0]*100
